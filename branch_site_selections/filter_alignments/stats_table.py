...
CDS_DIR = Path(f"{PATH_TO_PROJECT}/initial_data/cds_TOGA")
...
OUT_PHYLIP_DIR.mkdir(exist_ok=True)
...
SUMMARY_OUT = f"{PATH_TO_PROJECT}/tmp/alignment_qc_summary.tsv"
...
foreground = set(pd.read_csv(f"{PATH_TO_PROJECT}/initial_data/foreground_species.tsv", sep = "\s+")["Assembly_name"].tolist())
...
for aln_file in CDS_DIR.glob("*.fa*"):
    ...
    for rec in records:
        ...
        if sp in allowed_species:
            ...
            species_map[sp_name].append(rec)
            if sp in foreground:
                foreground_sp_in_alignment.add(sp_name)
    ...
    # save fasta
    fasta_out = OUT_FASTA_DIR / aln_file.name
    SeqIO.write(kept_records, fasta_out, "fasta")

    # save phylip
    phylip_out = OUT_PHYLIP_DIR / (gene + ".phy")
    SeqIO.write(kept_records, phylip_out, "phylip-relaxed")
